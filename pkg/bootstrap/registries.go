...
// Non-exported local type alias so that we can define new methods
type registry templates.Registry
...
func (r *registry) Resolve(target authn.Resource) (authn.Authenticator, error) {
	...
	return r.getAuthenticatorForEndpoint(endpointURL)
}
...
func (r *registry) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	tlsConfig, err := r.getTLSConfigForEndpoint(endpointURL)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get TLS config from private registry configuration")
	}
...
// we only use the first endpoint from the mirror list. If no endpoint configuration is found,
// https is assumed.
...
// getAuthenticatorForEndpoint returns an Authenticator for a given URL. This uses the URL from the
// mirror endpoint list, NOT the URL from the request. If no configuration is present, Anonymous
// authentication is used.
func (r *registry) getAuthenticatorForEndpoint(endpoint *url.URL) (authn.Authenticator, error) {
	if config, ok := r.Configs[endpoint.Host]; ok {
		if config.Auth != nil {
...
// getTLSConfigForEndpoint returns TLS configuration for a given URL. This uses the URL from the
// mirror endpoint list, NOT the URL from the request.  This is cribbed from
// https://github.com/containerd/cri/blob/release/1.4/pkg/server/image_pull.go#L274
func (r *registry) getTLSConfigForEndpoint(endpoint *url.URL) (*tls.Config, error) {
